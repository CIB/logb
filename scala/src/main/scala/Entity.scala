...
abstract class Entity {
  ...
  def substitute(env: Map[String, Entity]): Entity
...
case class EStatement(statementTypeC: String, argumentsC: Map[String, Entity]) extends Entity {
  ...
  override def substitute(env: Map[String, Entity]): Entity = {
    val newArguments = arguments.mapValues(entity => entity.substitute(env))
    ...
    other match {
      case EStatement(otherType, otherArgs) =>
        if (otherType == statementType) {
          ...
          resultingEnvironments.foldLeft(init)(EntityHelpers.combineEnvironmentsFoldFunction)
        } else {
          None
        }
      case _ => None
